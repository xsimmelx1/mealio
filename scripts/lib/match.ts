...
export function matchProduct(
  spec: IngredientSpec,
  products: RawProduct[],
  opts: { offers?: boolean; ignoreCategory?: boolean; anySale?: boolean } = {},
): MatchResult | null {
  const terms = spec.terms.map(lc);
  const excludes = (spec.exclude ?? []).map(lc);
  const prefer = (spec.preferBrands ?? ['ja!', 'rewe']).map(lc);
  const cats = opts.ignoreCategory ? undefined : spec.categories;
  const wantOffers = opts.offers === true;

  const candidates = products.filter((p) => {
    if (!opts.anySale && p.sale !== wantOffers) return false; // Normal- bzw. (offers) Angebotsprodukte
